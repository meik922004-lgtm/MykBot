...
class CombatView(discord.ui.View):
    def __init__(self, cog_instance):
        # Đặt timeout=None và gắn custom_id để nút bấm tồn tại vĩnh viễn
        super().__init__(timeout=None)
        self.cog = cog_instance

    @discord.ui.button(label="Attack", style=discord.ButtonStyle.danger, emoji="⚔️", custom_id="btn_boss_attack")
    ...
    @discord.ui.button(label="Toggle Auto-Attack", style=discord.ButtonStyle.primary, emoji="🤖", custom_id="btn_boss_auto")
    ...
    @discord.ui.button(label="Protect (45s CD)", style=discord.ButtonStyle.secondary, emoji="🛡️", custom_id="btn_boss_protect")
...
class RPGSystemCog(commands.Cog):
    ...
    def __init__(self, bot):
        ...
        # Đảm bảo loop update Boss chạy ngầm
            ...
            if url := c.get("webhook_url"):
                try:
                    async with aiohttp.ClientSession() as s:
                        ...
                        # Gắn trực tiếp UI CombatView vào tin nhắn
    ...
    async def live_boss_update_loop(self):
        ...
        for msg_info in active_messages:
            try:
                ...
                if webhook:
                    # Việc edit embed sẽ tự động giữ lại CombatView UI ban đầu
    ...
    async def handle_manual_attack(self, interaction: discord.Interaction):
        # Trả lời ẩn cho riêng người bấm nút để tránh làm trôi thông báo Boss chung
        await interaction.response.defer(ephemeral=True) 
