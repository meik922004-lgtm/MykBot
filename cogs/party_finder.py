...
# ==========================================
# CONFIGURATION: TỪ KHÓA DUNGEON VÀ ROLE PING
# ==========================================
DUNGEON_ROLES = {
    "nanomon": "<@&111111111111111111>",
    "kimera": "<@&222222222222222222>",
    "myotismon": "<@&333333333333333333>",
    "raid": "<@&444444444444444444>"
}

def get_ping_role(dungeon_name: str) -> str:
    name_lower = dungeon_name.lower()
    for key, role_ping in DUNGEON_ROLES.items():
...
async def build_lobby_embed(page: int = 1, search_query: str = None):
    ...
    for party in active_parties:
        # SỬA LỖI 1: Sử dụng .get() phòng hờ và tự bỏ qua nếu phòng đó bị lỗi thiếu dữ liệu leader_id
...
class JoinInputIGNModal(discord.ui.Modal):
    ...
    async def on_submit(self, interaction: discord.Interaction):
        ...
        await players_col.update_one({"user_id": user_id}, {"$set": {"ign": ign_val}}, upsert=True)
        
        # SỬA LỖI 3: Trích xuất Gear đi xuyên qua Object bọc ngoài 'my_stats' theo đúng Mongo Compass của bạn
        player_profile = await players_col.find_one({"user_id": user_id})
        player_gear = "Chưa cập nhật"
        if player_profile and "my_stats" in player_profile:
            player_gear = player_profile["my_stats"].get("gear", "Chưa cập nhật")
        ...
        await interaction.response.send_message("✅ Your join request & profile have been submitted to the leader!", ephemeral=True)
...
class CreatePartyModal(discord.ui.Modal):
    ...
    start_time = discord.ui.TextInput(label="Expected Start Time", placeholder="e.g., 20:00 / In 10 mins / ASAP", default="ASAP", max_length=30)
    ...
    async def on_submit(self, interaction: discord.Interaction):
        ...
        broadcast_embed = discord.Embed(
            ...
            description=f"**Leader:** <@{user_id}>\n"
                        f"**Slots Available:** `1/{max_slots}`\n"
                        f"**Start Time:** ⏰ `{new_party['start_time']}`\n"
                        f"**Requirements:** {new_party['requirements']}",
        ...
        if chan:
            role_ping_str = get_ping_role(new_party['dungeon'])
            await chan.send(content=role_ping_str, embed=broadcast_embed, view=public_view)
...
class SearchDungeonModal(discord.ui.Modal):
    query = discord.ui.TextInput(label="Enter dungeon keyword", placeholder="e.g., Castle / Hard...", required=False)
...
class LobbyView(discord.ui.View):
    ...
    async def prev_page(self, interaction: discord.Interaction, button: discord.ui.Button):
        if self.page > 1:
            self.page -= 1
            # SỬA LỖI 2: Thêm `await` trước build_lobby_embed ở đây
    ...
    @discord.ui.button(label="🔍 Filter", style=discord.ButtonStyle.primary, custom_id="lobby_filter")
    ...
    @discord.ui.button(label="⚙️ Manage My Party", style=discord.ButtonStyle.danger, custom_id="lobby_manage")
...
class PartyFinder(commands.Cog):
    ...
    async def party_lobby(self, interaction: discord.Interaction):
        ...
        try:
            ...
            # Nếu webhook hết hạn hoặc lỗi nghiêm trọng
            try:
                await interaction.followup.send(f"❌ Lỗi hệ thống: {e}", ephemeral=True)
            except discord.errors.NotFound:
                print(f"[ERROR LOG] Khách hàng gặp lỗi Webhook không phản hồi: {e}")
